...
### Fixed
- The voice invitation picker is now anchored directly to the invite icon and opens beside it, independently of the sidebar width.
...
### Added
- Incoming voice-channel invitations now play a distinct sound for the invited user.
- Invitations are also delivered as direct messages with persistent **Accept** and **Decline** actions, protected by the existing five-minute expiration.
...
### Fixed
- The phone-hangup control now uses the same compact rectangular dimensions and corner radius as the channel settings button.
